...
$checkEvent = mysqli_prepare($conn, "SELECT eventID FROM events WHERE eventID = ?");
...
$checkRegistration = mysqli_prepare($conn, "SELECT 1 FROM registrations WHERE attendeeID = ? AND eventID = ?");
